...
for i in time_arr:
	for j in i:
		for k in j:
			...
			# time.sleep(0.1)
			f.write(str(grid)+","+str(hour)+","+str(k)+","+str(j[k][0])+","+str(j[k][1])+","+str(j[k][2])+","+str(j[k][3])+","+str(average_crime(get_neighbors(grid),hour,k))+","+str(j[k][4])+"\n")
