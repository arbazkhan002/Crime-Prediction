...
f.close()
f = open("monthly_train_not_hourly_t4.csv","w")
...
for i in time_arr:
	...
	for k in i:
		...
		# if dayd%100 ==1:
			# print 'hello hello'
		f.write(str(grid)+","+str(k)+","+str(i[k][0])+","+str(i[k][1])+","+str(i[k][2])+","+str(i[k][3])+","str(i[k][4])+","+str(average_crime(get_neighbors(grid),k))+","+str(i[k][5])+"\n")
