monthly_data = csvread('monthly_train_not_hourly_t9.csv');
% format of this data:
...
for i=1:900
    H(i) = nnz(monthly_data(monthly_data(:,1)==i,14));
    C(i) = length(monthly_data(monthly_data(:,1)==i,14))-H(i);
end
C=C';
H=H';
...
for i=1:length(monthly_data)
   norm_monthly_data(i,14) = norm_monthly_data(i,14)*out(norm_monthly_data(i,1));
end

filenametemplate = 'monthly_nothourly_t9/Result%3.3d.csv';
for i=1:900
    ...
    if ~isempty(monthly_i ~= 0) && length(monthly_i(:,1)) > 20
        ...
        Class = knnclassify(testing(:,1:11),training(:,1:11),training(:,12));
        Results = [testing(:,12),Class];
        TP=0;
        TN=0;
        FP=0;
        FN=0;
...
%         xlabel('');
        ...
        fig = figure(gcf);
        saveas(fig,sprintf('monthly_nothourly_t9/result%03d',i),'jpg');
        close(fig);
    end
end

Accuracy = Accuracy(Accuracy(:,1)~=0,:);
dlmwrite('accuracy_monthly_t9.csv',Accuracy);